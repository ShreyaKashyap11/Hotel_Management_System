...
# PowerShell script to create the Hotel Management System project tree and zip it.
# Save this file and run in PowerShell:
#   powershell -ExecutionPolicy Bypass -File .\create_project_and_zip.ps1
# The script will create hotel-management.zip in the same folder.
...
$dirs = @(
  "backend/src/main/java/com/example/hotelmanagement/model",
  "backend/src/main/java/com/example/hotelmanagement/repository",
  "backend/src/main/java/com/example/hotelmanagement/service",
  "backend/src/main/java/com/example/hotelmanagement/controller",
...
# backend/pom.xml
Write-TextFile "backend/pom.xml" @'
...
Write-TextFile "backend/src/main/java/com/example/hotelmanagement/HotelManagementApplication.java" @'
...
# backend model
Write-TextFile "backend/src/main/java/com/example/hotelmanagement/model/Hotel.java" @'
...
# backend repository
Write-TextFile "backend/src/main/java/com/example/hotelmanagement/repository/HotelRepository.java" @'
...
# backend service
Write-TextFile "backend/src/main/java/com/example/hotelmanagement/service/HotelService.java" @'
...
# backend controller
Write-TextFile "backend/src/main/java/com/example/hotelmanagement/controller/HotelController.java" @'
...
Write-TextFile "backend/src/main/resources/application.properties" @'
server.port=8080
...
Write-TextFile "backend/Dockerfile" @'
# Build stage
...
Write-TextFile "backend/.dockerignore" @'
target
.git
.idea
.vscode
...
Write-TextFile "k8s/backend-deployment.yaml" @'
apiVersion: apps/v1
kind: Deployment
metadata:
  name: hotel-backend
...
Write-TextFile "frontend/src/components/HotelForm.jsx" @'
import React, { useEffect, useState } from "react";
import api from "../api";
...
Write-TextFile "frontend/Dockerfile" @'
# Build stage
...
Write-TextFile "frontend/.dockerignore" @'
node_modules
dist
.git
'@
...
Write-TextFile "k8s/frontend-deployment.yaml" @'
apiVersion: apps/v1
kind: Deployment
metadata:
  name: hotel-frontend
...
# README.md (wrapped with markdown fences inside file as requested earlier)
